...
const form = () => {
  ...
  useEffect(() => {
    if (query.id) {
      axios.get("/api/diretores/" + query.id).then((resultado) => {
        console.log(resultado.data);

        const diretor = resultado.data;

        for (let atributo in diretor) {
          setValue(atributo, diretor[atributo]);
        }
      });
    }
  }, [query.id]);
  ...
  function salvar(dados) {
    axios.put("/api/diretores/" + query.id, dados);
    push("/diretores");
  }
