...
const form = () => {
  ...
  useEffect(() => {
    if (query.id) {
      axios.get("/api/filmes/" + query.id).then((resultado) => {
        console.log(resultado.data);

        const filme = resultado.data;

        for (let atributo in filme) {
          setValue(atributo, filme[atributo]);
        }
      });
    }
  }, [query.id]);
  ...
  function salvar(dados) {
    axios.put("/api/filmes/" + query.id, dados);
    push("/filmes");
  }
      ...
      <Pagina>
        <Form>
          ...
          <Form.Group className="mb-3" controlId="sinopse">
            <Form.Label><strong>Sinopse: </strong></Form.Label>
            <Form.Control
          ...
          <Form.Group className="mb-3" controlId="etaria">
            <Form.Label><strong>Faixa Etária: </strong></Form.Label>
            <Form.Control
              mask="99"
